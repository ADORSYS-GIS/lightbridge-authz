...
This exchange is also where project context is **sealed into the JWT** for the human plane — but only `account_id`/`project_id`, not `role`/`quota_tier`/`project_quota`. The consequence is that switching project means requesting a new token, not sending a different header. `role`/`quota_tier`/`project_quota` are deliberately kept off the token and instead ride the Authorino introspection response (`docs/governance-model-and-enforcement.md`, "Why introspection and not claims"): a roster or quota change would otherwise only take effect at the human plane's next token refresh, whereas introspection is cached just 30s per key, so a lead's `quota_tier` edit or an owner's `project_quota` bump propagates within that window instead of a token lifetime.
