...
- `account_memberships` (belongs to `accounts`): maps OAuth2/JWT `subject` identities to account membership/admin rights.
...
### Membership & Tenancy Rules

- Account creation does **not** accept an explicit owners/admins payload; the authenticated JWT `sub` becomes the initial account member/admin.
- Account access control is membership-based (`account_memberships`), and multi-user accounts are supported by adding/removing memberships.
- Guardrail: if an account loses all memberships, it is deleted, including related projects and API keys.
- Multi-tenancy: always scope reads/writes by tenant/account context when possible (prefer account-constrained queries, then project/api-key joins under that account).
- All list surfaces (REST API and MCP tools) should expose pagination (`offset`, `limit`) with bounded limits.
...
- CRUD API (`authz-api`)
  ...
  - List endpoints support pagination using `offset` and `limit` query/input parameters.
...
The Postgres-backed `lightbridge-authz-api-key` tests (membership access control, cascade deletion guardrail, rotate transaction safety, project limits) are guarded by the `it-tests` feature so they only compile/run when requested. This keeps the default `cargo test` free of database setup, and lets us treat these as Docker-backed integration tests.
