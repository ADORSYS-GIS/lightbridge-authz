...
| `oauth2.jwks_url` | `String` | **Required** | JWKS endpoint used to verify **inbound** bearer tokens — tokens presented TO this service (Keycloak in local compose; `authz-idp`'s own JWKS in prod, per the platform rule in `docs/local-testing.md`). On `authz-idp` its only consumer is the RFC 8693 `subject_token` check (`BearerTokenService` → `handle_token_exchange`). It is **not** the upstream IdP's key set: `KeycloakRelyingParty` takes the keys that verify Keycloak's ID tokens from the discovery document's own `jwks_uri`, so this key never governs the RP leg | Every bearer-protected request fails to validate. It used to ALSO take the RP leg down — `discover()` once required `metadata.jwks_uri == oauth2.jwks_url`, so pointing this at `authz-idp`'s own JWKS broke 100% of fresh logins (prod outage 2026-09-01); that coupling is removed |
| `oauth2.jwks_ca_bundle_path` | `Option<String>` | default `None` (lightbridge-authz#625) | Path to a PEM CA bundle trusted, in ADDITION to the platform trust store, for `jwks_url` when it resolves to a private in-cluster endpoint (e.g. `authz-idp`'s own Service DNS name) instead of hairpinning back out through the public ingress. Scoped to ONLY the JWKS HTTP client (`reqwest::ClientBuilder::add_root_certificate` via `lightbridge_authz_bearer::trust_ca_bundle`, `crates/lightbridge-authz-bearer/src/jwks_client.rs`) — never `SSL_CERT_FILE`, which would also change what Keycloak discovery trusts. Wired at BOTH JWKS-fetching sites: `BearerTokenService::new` (`crates/lightbridge-authz-bearer/src/lib.rs`) and `KeycloakRelyingParty::new` (`crates/lightbridge-authz-rest/src/relying_party.rs`, which reuses the same client for the JWKS fetch `validate_id_token` needs) | Unset (the common case) is byte-identical to before this field existed. An unreadable path or a bundle with no parseable PEM certificate is a hard construction-time failure naming the path — never a silent fallback to the default client |
...
| `oauth2.token_exchange.refresh_ttl_seconds` | `i64` | default `2_592_000` (30 days) | Server-wide per-**token** lifetime, reset on every rotation (`new_row.expires_at = now + <effective refresh_ttl_seconds>`, `oauth2_op/store.rs`) — this is not a session-level ceiling by itself; see `refresh_absolute_ttl_seconds` immediately below for the field that actually bounds a session. **Overridable per client** — see `oauth2.clients[].refresh_ttl_seconds` below; a client with no override uses this value | `<= 0` → startup fails (`oauth2_op::refresh_ttl::validate_client_refresh_ttls`, called from `build_token_exchange_state`) |
| `oauth2.token_exchange.refresh_absolute_ttl_seconds` | `i64` | default `7_776_000` (90 days) | Server-wide absolute cap on a refresh-token **chain** (every token minted across one rotation lineage), not the individual token above. Set once, at chain birth (the offline-scope exchange grant), to `now + <effective refresh_absolute_ttl_seconds>` (`chain_expires_at`), and inherited unchanged by every subsequent rotation — this is what stops a session that keeps refreshing before every individual `expires_at` from living forever. See §4 below for the full chain/status model. **Overridable per client** — see `oauth2.clients[].refresh_absolute_ttl_seconds` below | `<= 0` → startup fails. So does an EFFECTIVE (override-or-global) `refresh_ttl_seconds` greater than the EFFECTIVE `refresh_absolute_ttl_seconds` for ANY configured client, or for the bare global pair when a client leaves both unset — `oauth2_op::refresh_ttl::validate_client_refresh_ttls` runs at startup for every client plus the global default, naming the offending client id (or `<global>`) and both values in the error |
...
| `oauth2.clients[].refresh_ttl_seconds` | `Option<i64>` | default `None` | Per-client override of `oauth2.token_exchange.refresh_ttl_seconds` (the per-**token** lifetime). `None` falls back to that server-wide value. Resolved once at startup into `ConfigClientStore`'s per-client lookup, never re-read per request | EFFECTIVE value (this override, or the global fallback) `<= 0`, or greater than the client's EFFECTIVE `refresh_absolute_ttl_seconds` below → startup fails, naming this client's id and both values |
| `oauth2.clients[].refresh_absolute_ttl_seconds` | `Option<i64>` | default `None` | Per-client override of `oauth2.token_exchange.refresh_absolute_ttl_seconds` (the absolute chain cap). `None` falls back to that server-wide value. Deliberately a SEPARATE override from `refresh_ttl_seconds` above — overriding only one of the pair without the other is exactly the half-working config trap the startup check below catches | EFFECTIVE value `<= 0` → startup fails. A client with a longer per-token TTL than its (possibly still-global) absolute cap is refused at startup instead of silently minting tokens the cap kills before their own expiry |
