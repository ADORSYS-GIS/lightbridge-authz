...
| `oauth2.relying_party.issuer`, `.client_id`, `.callback_url` | `String` | required when the RP block is configured | Keycloak discovery/ID-token issuer, ID-token audience, and the one exact Keycloak callback URL | An issuer mismatch, token endpoint error, signature/`kid`/issuer/audience/`iat`/nonce failure, or callback-state failure refuses completion without approving a device code or issuing a browser cookie; multi-audience ID tokens must set `azp` to this client |
...
| `authorization_endpoint`, `userinfo_endpoint`, `response_types_supported`, `response_modes_supported`, `claims_supported` | **Stale row, not updated in this pass (see note below).** ADR-0019/#467 added `authorization_endpoint`/`response_types_supported`/`response_modes_supported`/`code_challenge_methods_supported` back to `DiscoveryDocument`, gated on the authorization-code capability — they are no longer structurally absent. `userinfo_endpoint`/`claims_supported` remain genuinely unimplemented. | — |
