...
| Epic: RBAC beyond admin and user (org-admin, team-admin, auditor) | **Partial** | [#262](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/262) (open), children [#263](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/263)–[#265](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/265) | #656 delivered the *assignment* half (a grant is a row). The *role model* half — scoped admins, an auditor role — is still open (#262–#264). **#265 done** (#177 closed with regression coverage): `QuotaTiers::is_allowed` is wired on all its write paths, and `quota_tier_it_tests.rs` is what fails if a write path stops calling it; the doc-tests on `is_allowed` pin the predicate's own fail-closed semantics (they cannot observe a dropped caller). `every_mapped_op_id_is_refused_without_its_required_permission` (`rpc_router_tests.rs`) + `rbac_gate_refuses_every_budget_op_without_its_permission` (`budget_router_tests.rs`) assert that every op-id in `MAPPED_OP_ID_PERMISSIONS` is refused by the RBAC gate when its required permission is absent — and, via the full-permission control half of each iteration, that the refusal is attributable to the missing permission and not to a deny-everything gate. |
...
| `budget schedule create` / `list` CLI | Done | [#702](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/702); [`budget-cli.md`](./budget-cli.md#budget-schedule--authoring-the-rule-instead-of-the-money); `app/lightbridge-authz/src/budget_schedule_cmd.rs`, `crates/lightbridge-authz-budget/src/reset_schedule_resolve.rs`; `budget_schedule_cmd_tests.rs` (8) | The `grant` argument one level up: `createBudgetResetSchedule`/`updateBudgetResetSchedule` need `permBudgetScheduleManage`, a HUMAN platform role, and ADR-0030 gives a service token no `roles` claim — so a Job could not author a schedule at all. Delegates to `ResetScheduleRepo`, so `validate_shape` and the window derivation are shared with the RPC, not re-implemented. Idempotent on `--name`; a same-named row with a **different** shape is a refusal naming the field, never a silent success. `--dry-run` resolves and writes nothing; `--enable` is a separate `UPDATE`, because the domain layer creates every schedule disabled (ADR-0032 D8). |
...
| Starting grant at account creation | **Done** | [#697](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/697); [ADR-0015 Decision 9](./adr/0015-refill-amounts-are-admin-configured-policy-ranges.md#amendment-2026-09-05--decision-9-the-starting-grant-is-booked-at-account-creation); `crates/lightbridge-authz-budget/src/starting_grant*.rs`, `handlers/accounts.rs`, migration `20260905000001`; `starting_grant_tests.rs` + `starting_grant_it_tests.rs` | `AuthzStoreImpl::create_account` — the one handler behind the RPC procedure, the MCP tool and the console bootstrap — now books one `automatic` grant for the current period, idempotent on `budget-start-<period>-<account_id>`, through `BudgetRepo::grant`, and touches the snapshot. **Amount = what `effective_schedule(account)` would reset it to**, policy `starting_amount_micros` only when no schedule covers the account, so the next window is a `delta = 0` no-op instead of a `correction`. Caveat pinned by test: an account has no `billing_plan` until it has a project, so a plan-scoped schedule does not cover it at creation and the policy amount fires — keep the two aligned. Funding every account also surfaced a latent bug — a granted account could never be hard-deleted (`NO ACTION` FK + ADR-0009's append-only trigger, an opaque `500`); the budget tables now cascade and the trigger has one exemption, a DELETE whose owning account row is already gone, i.e. the cascade and nothing else. |
...
| `starting_amount_micros` ($15) vs the live `$8` schedule | **Done** | [#702](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/702); [`budget-cli.md` §the $8-vs-$15 rule](./budget-cli.md#the-8-vs-15-rule-match-the-operative-schedule-not-the-policy-default); `starting_grant_tests.rs` (2 new) | Fixed by **option (b)**: a `global`-scoped `mode: reset` schedule at `8000000` (`"Global refill $8"`, weekly, ISO weekday 1, aligned to the same 2026-09-07 tick as `"Refill $8"`), authored on prod with `budget schedule create`. A global schedule covers an account from its first second, so `effective_schedule` now returns `Some` for every account and `StartingAmount::PolicyDefault` — the only production reader of `starting_amount_micros` (`starting_grant.rs:101`) — is unreachable. The policy is deliberately **left at $15**: it is the no-schedule-at-all answer, and lowering it would also cut the `fail_closed_floor_micros <= starting_amount_micros` headroom `rule_data::validate` enforces. Precedence is unchanged and pinned: the `billing_plan=free` schedule still wins for free-plan accounts. |
...
| Item | State | Evidence | Notes |
|---|---|---|---|
| Public IDE collector → usage DB | **Partial; not deployed** | [governance#358](https://github.com/ADORSYS-GIS/lightbridge-governance/issues/358); [request replay prerequisites](./collector-request-retry.md) | Canonical replay source validation, natural-key dedup, trusted account/user binding (resource attributes now win over payload at every signal level), and rollup source preservation (EAIG-only spend, source-aware rollup) are prepared and tested. The governance chart's collector→usage exporter leg is code-complete and validated against the real collector binary for all three public collectors (`aiCliOtel`/`claude-code`, `opencodeOtel`/`opencode`, and the new `codexOtel`/`codex` — the "third collector" a values-repo comment already called for, unblocking Codex/VS Code Copilot's previously-401ing `lightbridge-api-key` credential), all gated off by default. Remaining: enabling each leg in the values repo and a real-client/scoped-query proof per source; VS Code Copilot's day/seat-grain ingestion is a separate, still-unresolved shape and must not share `codexOtel`'s `source: codex` labeling. No collector usage leg enabled. |
...
| `usage_events` retention | **Done (interim)** | [#549](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/549); `migrations-usage/20260903000004`; `crates/lightbridge-authz-usage/src/retention.rs` | Ingest no longer writes the `attributes` blob, and a background job rolls rows older than `retention.raw_days` (default 90) into `usage_events_daily`, then purges them, in one statement (`20260903000004` + `retention.rs`). The `DROP COLUMN` itself ships in a **follow-up release** (`20260903000003`, per ADR-0031's expand/contract rule — the drop is separate from the code that stops writing it). `spend_for_account` reads raw UNION ALL rollup. Interim on a table #581's PR-1b rewrites; see the D6/D7 deviation note in `plans/0581-…md`. |
...
| Named KPI aggregates per measure, refresh proven, never spanning grains | **Done (plain Postgres)** | [#587](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/587); `migrations-usage/20260918000001`; `crates/lightbridge-authz-usage/src/aggregate_refresh.rs` | Owner decision (2026-09-21): **no TimescaleDB** — the "continuous aggregates" are plain-Postgres materialized views (one per KPI measure, each on one grain table) refreshed by the usage service's `aggregate_refresh` background loop (Option A, mirroring `retention_loop`). Four named aggregates, all on the day/seat grains; money discipline (unknown-cost rows counted separately, never 0). The day-facts and seat query endpoints route to the aggregates (raw fallback when absent); the execution endpoint spans grains so it stays raw (no hourly aggregate, since no query would read one). Existence/refresh/money/EXPLAIN tests in `aggregate_refresh_it_tests.rs`. |
...
| LoC ratchet does not ratchet | **Done — this PR** | [#622](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/622); self-tests `.github/actions/loc-gate/tests/` wired as `loc-gate-self-test`; raise override = `loc-baseline-raise` PR label + `/.github/loc-baseline.json` CODEOWNERS rule | Previously: the gate's contract is "grandfathered files may be touched but must not grow", but the committed baseline was edited by the same PR that grew the code — including twice after #622 was filed ([#655](https://github.com/ADORSYS-GIS/lightbridge-authz/pull/655), [#667](https://github.com/ADORSYS-GIS/lightbridge-authz/pull/667)). Now the loc-gate diffs `.github/loc-baseline.json` between base and head and fails any PR where an entry rises or a new key appears without an explicit, visible override label; lowering stays free. |
...
| `budget` operator guide (`grant` + `schedule`) | Done | [`budget-cli.md`](./budget-cli.md); [#695](https://github.com/ADORSYS-GIS/lightbridge-authz/pull/695) `b0b7904`, [#702](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/702) | Flags, refusals, idempotency, the Job pattern, the $8-vs-$15 rule (with its 2026-09-05 amendment), the never-raw-SQL rule, and — since #702 — `budget schedule create`/`list`: name-keyed idempotency, the create-disabled/enable-separately contract, and the precedence a `global` row does *not* take over. Linked from `rbac.md`, `docs/README.md`, `runbooks/README.md`. |
...
1. **[#549](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/549) — `usage_events` retention.** The retention/rollup job landed and ingest no longer writes the `attributes` blob (see §5); the `DROP COLUMN` ships in a follow-up release per ADR-0031, and the remaining open item is the #581 PR-1b rewrite of the interim table, plus the D6/D7 reconciliation recorded in `plans/0581-…md`.
