...
| Epic: RBAC beyond admin and user (org-admin, team-admin, auditor) | **Partial** | [#262](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/262) (open), children [#263](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/263)–[#265](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/265) | #656 delivered the *assignment* half (a grant is a row). The *role model* half — scoped admins, an auditor role — is still open (#262–#264). **#265 done** (#177 closed with regression): `QuotaTiers::is_allowed` is wired on all 4 write paths (`quota_tier_it_tests.rs`); `every_mapped_op_id_is_refused_without_its_required_permission` (`rpc_router_tests.rs`) + `rbac_gate_refuses_every_budget_op_without_its_permission` (`budget_router_tests.rs`) cover all 57 mapped op-ids with mutation-style gate tests; doc-tests on `QuotaTiers::is_allowed` are the prospective CI guard against a future uncalled check function. |
