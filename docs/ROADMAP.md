...
| `usage_events` retention | **Done (interim)** | [#549](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/549); `migrations-usage/20260903000003..04`; `crates/lightbridge-authz-usage/src/retention.rs` | The `attributes` blob is dropped at ingest (`20260903000003`, catalog-only) and a background job rolls rows older than `retention.raw_days` (default 90) into `usage_events_daily`, then purges them, in one statement (`20260903000004` + `retention.rs`). `spend_for_account` reads raw UNION ALL rollup. Interim on a table #581's PR-1b rewrites; see the D6/D7 deviation note in `plans/0581-…md`. |
...
1. **[#549](https://github.com/ADORSYS-GIS/lightbridge-authz/issues/549) — `usage_events` retention.** The retention/rollup job and the `attributes` drop landed (see §5); the remaining open item is the #581 PR-1b rewrite of the interim table, and the D6/D7 reconciliation recorded in `plans/0581-…md`.
