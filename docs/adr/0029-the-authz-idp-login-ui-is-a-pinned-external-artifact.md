...
> hand-listed) and verified there by `scripts/verify-routes-manifest.mjs`. `static_assets.rs` reads
> it at startup and 404s every `/ui` path outside it. Its absence is fail-closed, not fail-open: only
> `/` serves `index.html`, deep links 404, and the process does not panic — the same
> degrade-don't-crash posture `missing_static_dir_degrades_to_404_without_panicking` has always held
> this module to. `.github/actions/stage-authz-ui` now refuses a pin whose bundle lacks it.
>
