...
| `POST /oauth2/token` (`grant_type=client_credentials`) | Implemented and unconditional (ADR-0030, #534). RFC 6749 §4.4 M2M grant, intercepted before upstream dispatch (mirroring the pre-existing device-code intercept), `private_key_jwt`-only (`OauthClientType::Service`, behaviorally identical to `Confidential`). Mints `sub = "svc:<client_id>"`, no `roles` claim (zero RBAC permissions against every RPC op-id), no refresh or ID token. Advertised in discovery unconditionally, in the same block as the token-exchange/refresh grants, never gated on whether any `oauth2.clients` entry lists it. A disjoint machine plane on the same route — not part of this document's browser/device client model. |
...
**`client_credentials` has shipped** (ADR-0030, #534) — it was previously listed here as a non-goal
pending "a concrete machine-to-machine use case with a defined client identity, audience, and
authorization policy," and #534 met exactly that bar. See "What is implemented now" above for the
current state: RFC 6749 §4.4, `private_key_jwt`-only, disjoint from this browser/device roadmap's
own client model.
