...
| [reclaim-usage-events-space.md](./reclaim-usage-events-space.md) | The `usage` database is consuming more volume than the retention window should allow, or you need to physically reclaim the space from the `attributes` column (#549 AC5) |
...
| **Fund an account by hand** (`lightbridge-authz budget grant`), or **author a reset schedule** (`budget schedule create`) | [`docs/budget-cli.md`](../budget-cli.md) | It is an operator *tool*, not a symptom. The runbook that sends you to it is [budget-remaining-snapshot.md](./budget-remaining-snapshot.md) §7, when an account reads `known: true, remaining <= 0` because it was never granted anything |
