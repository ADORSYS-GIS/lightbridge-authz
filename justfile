...
# Seed usage_events with realistic multi-day, multi-project data (lightbridge-authz#528).
# Brings up the usage stack, then drives the real OTLP ingest endpoint. Idempotent:
# truncates usage_events first, so re-running never doubles figures.
seed-usage:
	@bash -ec 'set -euo pipefail; cmd="docker compose -p lightbridge-authz -f compose.yaml"; ${cmd} up -d timescaledb authz-usage-migrate authz-usage; sleep 5; python3 scripts/seed-usage-events.py'
...
# `spend_query_it_tests`, `scope_ownership_it_tests`, `seed_it_tests`, `execution_grain_it_tests`,
# `execution_query_it_tests`, `day_seat_grain_it_tests`, `cutover_verify_it_tests`,
# `day_grain_ingest_it_tests`, `execution_grain_ingest_it_tests`, `seat_query_it_tests` --
# #570/#578/#528/#582/#583/#726/#588/#728) run
# against this SAME
...
# image decision. `day_seat_grain_it_tests` carries 4 `#[ignore]`d tests that need a real
# Timescale-enabled Postgres for exactly that reason (hypertable/compression/retention
# assertions) -- they report as ignored here, not passed or failed; the other 15 need no
# Timescale-specific behavior and run for real.
# The usage it-tests guard is PER-BINARY (each of repo_it_tests/spend_query_it_tests/
# scope_ownership_it_tests/retention_it_tests/seed_it_tests/execution_grain_it_tests/
# execution_query_it_tests/day_seat_grain_it_tests/cutover_verify_it_tests/
# day_grain_ingest_it_tests/execution_grain_ingest_it_tests/seat_query_it_tests must itself report
# >0 passed), not an aggregate
# total -- an aggregate would hide one binary silently reporting "0 tests, exit 0" as long as the
# other two still pass a nonzero count, the same "a skipped test is not a passing test" failure
# mode AGENTS.md warns about, just moved one level up from an individual test to an individual
# binary.
it-tests:
	@bash -ec 'set -euo pipefail; cmd="docker compose -p lightbridge-authz -f compose.yaml"; ${cmd} up -d postgresql redis; ${cmd} up authz-migrate --exit-code-from authz-migrate; trap "${cmd} down postgresql redis authz-migrate" EXIT; sleep 2; export DATABASE_URL="postgres://postgres:postgres@localhost:5432/lightbridge_authz"; export AUTHZ_REDIS_URL="redis://127.0.0.1:6379"; cargo test -p lightbridge-authz-api-key --features it-tests --tests; cargo test -p lightbridge-authz-budget --features it-tests --tests; cargo test -p lightbridge-authz-rest --features it-tests; cargo test -p lightbridge-authz --features it-tests --test mcp_tool_it_tests; check_binary() { log_file=$(mktemp); cargo test -p lightbridge-authz-usage-rest --features it-tests --test "$1" 2>&1 | tee "${log_file}"; passed=$(grep -oE "[0-9]+ passed" "${log_file}" | grep -oE "^[0-9]+" | tail -1 || true); echo "$1 passed: ${passed:-0}"; if [ -z "${passed}" ] || [ "${passed}" -eq 0 ]; then echo "$1 reported 0 passed tests -- a skip is not a pass"; exit 1; fi; }; check_binary repo_it_tests; check_binary spend_query_it_tests; check_binary scope_ownership_it_tests; check_binary retention_it_tests; check_binary seed_it_tests; check_binary execution_grain_it_tests; check_binary execution_query_it_tests; check_binary day_seat_grain_it_tests; check_binary cutover_verify_it_tests; check_binary day_grain_ingest_it_tests; check_binary execution_grain_ingest_it_tests; check_binary seat_query_it_tests'
