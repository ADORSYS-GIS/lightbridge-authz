...
#    recorded ceiling;
# 2. the baseline ratchet (ADORSYS-GIS/lightbridge-authz#622) — the baseline
#    ITSELF may not rise in the same change: every existing entry must be at or
#    below its base-branch value and no new key may appear unless the change
#    carries an explicit `loc-baseline-raise` PR label. Regenerating the
#    baseline to meet grown code is the exact hole this closes (#655, #667).
set -euo pipefail
...
# --- Override label (ADORSYS-GIS/lightbridge-authz#622) -----------------------
...
# --- Is a changed path a test file (under a /tests/ dir)? ----------------------
# Test files are excluded from the gate's fail decision but reported separately
# (lightbridge-authz#516). Files under a `tests/` directory are Cargo
# integration-test trees; they are measured and surfaced as a notice, never a
# failure.
is_test_file() {
  local file="$1"
  case "${file}" in
...
# --- Baseline ratchet (ADORSYS-GIS/lightbridge-authz#622) ----------------------
...
while IFS=$'\t' read -r status path newpath; do
  ...
  if is_test_file "${path}"; then
    echo "::notice file=${path}::test file ${path}: ${count} LoC (reported, not gated)"
    continue
  fi
