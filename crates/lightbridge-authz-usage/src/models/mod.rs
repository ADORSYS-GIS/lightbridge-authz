...
///
/// Mirrors the `CHECK (subject_kind IN (...))` constraint in the migrations. Extensible via a
...
impl SubjectKind {
    /// The SQL CHECK-constraint string that matches this variant.
...
/// ADR-0027 Decision 2. Adding a source requires no schema change: the `source` column is a TEXT
/// NOT NULL dimension, never a table name. Upsert on the natural key makes reprocessing a day
/// idempotent (ADR-0028 D22).
///
...
pub struct UsageDayFact {
    ...
    /// Provider-scoped user identity — join key per governance#185. `None` for org/repo-level facts.
    ...
    /// Cost in integer micro-USD. `None` = unknown. NEVER `Some(0)` as a default.
...
/// A seat-grain snapshot row — one per `(source, snapshot_day, subject_kind, subject_id, provider_user_id)`.
///
/// The table `usage_seat_snapshots` is hypertable-partitioned by `snapshot_day` and
/// source-dimensioned per ADR-0027 Decision 2. Upsert on the natural key makes reprocessing
/// a day idempotent (ADR-0028 D22).
///
