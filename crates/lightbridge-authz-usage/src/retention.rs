...
//! deletes them from the raw table, in one transaction.
//!
...
//! day is rolled up exactly once, as a whole day. This is what makes the rollup a plain `INSERT`
//! with no `ON CONFLICT`: a day is never partially rolled up, so it can never be inserted twice,
//! and the transaction's atomicity (rollup + delete commit together) makes a re-run idempotent --
//! a committed run leaves no raw rows for the days it rolled up, and a rolled-back run leaves no
//! rollup rows at all.
//!
...
/// A failed run is logged and the loop continues -- a retention hiccup must not take the server
/// down, and the next run retries.
pub async fn run_retention_loop(pool: Arc<PgPool>, config: RetentionConfig) {
    ...
    info!(
        "usage retention/rollup enabled: raw_days={}, interval={}s",
        config.raw_days, config.interval_seconds
    );
    let interval = std::time::Duration::from_secs(config.interval_seconds.max(1));
    loop {
        tokio::time::sleep(interval).await;
        match rollup_and_purge(&pool, config.raw_days).await {
            Ok(purged) => {
                if purged > 0 {
...
/// complete days) into `usage_events_daily`, then deletes them -- one transaction. Returns the
/// number of raw rows purged.
pub async fn rollup_and_purge(pool: &PgPool, raw_days: i64) -> Result<u64> {
    ...
    let lock_acquired = sqlx::query_scalar::<_, bool>("SELECT pg_try_advisory_xact_lock(549000001)")
        .fetch_one(&mut *tx)
        .await
