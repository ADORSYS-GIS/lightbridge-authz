//! Day-facts grain query handler (#727): `POST /usage/v1/usage/facts/query`.
...
use crate::models::day_fact::{DayFactQueryRequest, DayFactQueryResponse};
use axum::{
    Json,
    extract::State,
...
#[utoipa::path(
    post,
    path = "/usage/v1/usage/facts/query",
    request_body = DayFactQueryRequest,
    responses(
        (status = 200, body = DayFactQueryResponse),
...
pub async fn query_day_facts(
    ...
    headers: HeaderMap,
    Json(input): Json<DayFactQueryRequest>,
) -> Result<Response> {
    ...
    let (points, truncated) = state.repo.query_day_facts(&input).await?;

    Ok((
        StatusCode::OK,
        Json(DayFactQueryResponse { points, truncated }),
    )
        .into_response())
}
