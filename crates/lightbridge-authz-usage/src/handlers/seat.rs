//! Seat-grain query handler (#728): `POST /usage/v1/usage/seats/query`.
...
use crate::models::seat::{SeatSnapshotQueryRequest, SeatSnapshotQueryResponse};
use axum::{
    Json,
    extract::State,
...
#[utoipa::path(
    post,
    path = "/usage/v1/usage/seats/query",
    request_body = SeatSnapshotQueryRequest,
    responses(
        (status = 200, body = SeatSnapshotQueryResponse),
...
pub async fn query_seat_snapshots(
    ...
    headers: HeaderMap,
    Json(input): Json<SeatSnapshotQueryRequest>,
) -> Result<Response> {
    ...
    // Scope authorization via the shared gate (DaySeat model: user + all only).
    ...
    let (points, truncated) = state.repo.query_seat_snapshots(&input).await?;

    Ok((
        StatusCode::OK,
        Json(SeatSnapshotQueryResponse { points, truncated }),
    )
        .into_response())
}
