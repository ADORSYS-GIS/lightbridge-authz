...
--
-- ## Deploy sequencing (ADR-0031 expand/contract)
--
-- This migration is the "removal" half of #549 AC1 and MUST ship in a release AFTER the one that
-- stops writing `attributes` (the #549 retention PR). It is safe to apply only once every pod runs
-- code whose INSERT no longer binds `attributes`; applying it while an old pod still writes the
-- column would make that pod's batched INSERT fail outright and lose the whole event. The column
-- is nullable-with-default (`JSONB NOT NULL DEFAULT '{}'`), so the release that stops writing it
-- needs no migration at all -- this drop is the follow-up.
